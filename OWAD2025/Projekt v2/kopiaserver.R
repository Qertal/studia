...
server <- function(input, output, session) {
  ...
  delayed_top_n <- debounce(reactive({
    ...
    val <- max(1, min(val, 50))  # clamp between 1 and 50
    val
  }), millis = 300)
  ...
  output$ranking_plot <- renderPlotly({
    ...
    top_n_val <- delayed_top_n()  # zamiast input$top_n
    ...
    if (input$ranking_reference_country == "-- None --") {
      ...
      p <- ggplot(df, aes(x = Country, y = per_1M, fill = Direction, text = Country)) +
        geom_col() +
    ...
    if (input$ranking_reference_country == "-- None --") {
      ...
      ggplotly(p, tooltip = NULL, source = NULL) %>%
  ...
  observeEvent(plotly::event_data("plotly_click", source = "ranking_click"), {
    ...
    if (!is.null(clicked)) {
      ...
      selected_detail_case(input$ranking_case)  # zapisz wybraną kategorię
  ...
  output$global_pie <- renderPlot({
    ...
    df_sum <- df %>%
      summarise(across(Confirmed:Active, sum)) %>%
      pivot_longer(cols = c(Recovered, Deaths, Active),
